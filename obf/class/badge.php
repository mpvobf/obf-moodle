...
class obf_badge {
    ...
    /**
     * Gets courses related to a badge.
     * @return array
     * @throws dml_exception
     */
    public function get_badge_courses() {
        global $DB;

        $sql = 'SELECT occ.courseid FROM {local_obf_criterion_courses} AS occ 
                INNER JOIN {local_obf_criterion} AS loc
                ON (occ.obf_criterion_id = loc.id)
                WHERE loc.badge_id = :badge_id';

        $courses = $DB->get_records_sql($sql, array('badge_id' => $this->get_id()));
        return $courses;
